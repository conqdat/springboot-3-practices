...
BEGIN
RETURN QUERY
SELECT DISTINCT bu.name AS businessUnitName, COUNT(on_bench.employee_id) AS total
FROM employee e
    JOIN business_unit bu ON e.business_unit_id = bu.id
    JOIN (SELECT DISTINCT eobd.employee_id
...
BEGIN
RETURN QUERY
SELECT DISTINCT br.name AS branchName, COUNT(on_bench.employee_id) AS total
FROM employee e
...
BEGIN
RETURN QUERY
SELECT DISTINCT el_l.level AS levelName, COUNT(on_bench.employee_id) AS total
FROM employee e
    JOIN (SELECT el.employee_id, l.name AS level
...
BEGIN
RETURN QUERY
SELECT DISTINCT group_coe.coe AS coeName, COUNT(on_bench.employee_id) AS total
FROM employee e
    JOIN (SELECT t.id, coe.name AS coe
